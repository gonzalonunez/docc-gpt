...
I also haven't figured out what to do about re-running the model on a file that has already been documented. In some cases, it actually improves upon its initial choices. In others, it adds a random newline or some other comment. Maybe this can also be fixed with better prompting, or maybe this calls for structing the CLI a little more.

There is also quite a bit of other work needed to take this all of the way to a usable state, like exposing the ability to ignore certain files/subdirectories.
